...
class NewExpenses extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<NewExpenses> createState() {
...
class _NewExpensesState extends ConsumerState<NewExpenses> {
  ...
  void _submitExpenseData() {
    final enteredAmount = double.tryParse(_amountController
        .text); // tryParse("Hello") = null tryParse("1.13") = 1.13
      ...
      showDialog(
        ...
        builder: (ctx) {
          return AlertDialog(
            ...
            content: const Text(
                "Enter the valid title, amount and date or it must be not empty"),
            actions: [
              TextButton(
                onPressed: () {
                  Navigator.pop(ctx);
    ...
    // );
    ref.read(addExpenseProvider.notifier).addExpense(Expense(title: _titleController.text, amount: enteredAmount, date: _selectedDate!, category: _selectedCategory));
